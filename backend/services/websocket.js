...
class WebSocketService {
  ...
  setupEventListeners(socket) {
    ...
    socket.on('join-call', async (data) => {
      ...
      if (phoneNumber) {
        try {
          // セキュリティチェック: このユーザーがこの電話番号の通話にアクセス権があるか確認
          const normalizedPhone = this.normalizePhoneNumber(phoneNumber);

          // ユーザーIDと電話番号でCallSessionを検索（過去の通話も含めてアクセス権を確認）
          // 直近の通話を確認すれば十分
          const hasAccess = await CallSession.findOne({
          ...
          if (hasAccess) {
            ...
            console.warn(`[WebSocket] Security Warning: User ${socket.userId} attempted to join unauthorized call room for ${phoneNumber}`);
            socket.emit('error', { message: 'この通話へのアクセス権がありません' });
          }
        } catch (error) {
          console.error('[WebSocket] Error in join-call security check:', error);
        }
      } else {
    ...
    socket.on('get-call-status', async (data) => {
      ...
      try {
        ...
        // まず、twilioCallSidが設定されている（実際に発信された）通話を優先的に取得
        let latestCall = await CallSession.findOne({
          phoneNumber: { $in: [phoneNumber, normalizedPhone] },
          userId: socket.userId,
          twilioCallSid: { $exists: true, $ne: null }
        }).sort({ createdAt: -1 });

        // twilioCallSidがある通話が見つからない場合は、全ての通話から最新を取得
        if (!latestCall) {
          latestCall = await CallSession.findOne({
            phoneNumber: { $in: [phoneNumber, normalizedPhone] },
            userId: socket.userId,
          }).sort({ createdAt: -1 });
        }

        console.log('[WebSocket] 最新の通話セッション:', latestCall ? {
          id: latestCall._id,
          status: latestCall.status,
          phoneNumber: latestCall.phoneNumber,
          twilioCallSid: latestCall.twilioCallSid,
        ...
        if (latestCall) {
          ...
          socket.emit('call-status', {
            ...
            status: latestCall.status,
            callSid: latestCall.twilioCallSid,
            callId: latestCall._id.toString()
          });
        } else {
          // 通話履歴がない場合は終了ステータスを返す
          console.log('[WebSocket] 通話履歴なし - endedステータスを返却');
          socket.emit('call-status', {
            phoneNumber: normalizedPhone,
            status: 'ended',
            callSid: null,
  ...
  // 古い通話セッションをクリーンアップ
  // 処理中(calling等): 5分でタイムアウト
  // 待機中(queued): 3時間でタイムアウト（ゾンビ対策）
