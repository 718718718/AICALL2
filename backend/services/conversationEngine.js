...
class ConversationEngine {
  ...
  async getDefaultResponse(intent, callId) {
    ...
    // フォールバック用のデフォルト応答
    // closing 系は mediaStreamController.CLOSING_PHRASES と表現を揃える。
    // 「お忙しいところ恐れ入ります。それでは失礼いたします。」で必ず終わる。
    const fallbackResponses = {
      ...
      website_redirect: '承知しました。御社ホームページの問い合わせフォームからご連絡いたします。お忙しいところ恐れ入ります。それでは失礼いたします。',
      closing: '本日はありがとうございました。お忙しいところ恐れ入ります。それでは失礼いたします。',
  ...
  // 終話待機処理
  //
  // Legacy <Gather>/<Say> path. With <Play>/<Say> based TwiML, the closing
  // phrase is appended to the TwiML response and <Hangup> fires naturally
  // AFTER playback. This timer is only used as a fallback (no <Hangup> in the
  // TwiML, or test paths) and MUST be long enough that any in-flight TTS has
  // fully played out before we hangup.
  //
  // Previous behavior was Math.random()*500+700 (700-1200ms) which routinely
  // cut off the closing phrase mid-syllable. We now use a deterministic value
  // taken from LEGACY_CLOSING_DELAY_MS, defaulting to 2800ms which is enough
  // for a 30-character JP closing sentence at sonic-3 default speed.
  async handleClosingWithDelay(callId, callback) {
    const state = this.conversationStates.get(callId);
    if (!state) return;

    const waitTime = parseInt(process.env.LEGACY_CLOSING_DELAY_MS || '2800', 10);
    console.log(`[ConversationEngine] 終話待機(deterministic): ${waitTime}ms`);
